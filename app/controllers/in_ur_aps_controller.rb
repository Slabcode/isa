class InUrApsController < ApplicationController
  before_action :set_in_ur_ap, only: [:show, :edit, :update, :destroy]

  # GET /in_ur_aps
  # GET /in_ur_aps.json
  def index
    @in_ur_aps = InUrAp.all
  end

  # GET /in_ur_aps/1
  # GET /in_ur_aps/1.json
  def show
  end
  ...
  # POST /in_ur_aps.json
  def create
    @in_ur_ap = InUrAp.new(in_ur_ap_params)

    respond_to do |format|
      if @in_ur_ap.save
        format.html { redirect_to @in_ur_ap, notice: 'In ur ap was successfully created.' }
        format.json { render :show, status: :created, location: @in_ur_ap }
      else
        format.html { render :new }
        format.json { render json: @in_ur_ap.errors, status: :unprocessable_entity }
      end
    end
  end

  # PATCH/PUT /in_ur_aps/1
  # PATCH/PUT /in_ur_aps/1.json
  def update
    respond_to do |format|
      if @in_ur_ap.update(in_ur_ap_params)
        format.html { redirect_to @in_ur_ap, notice: 'In ur ap was successfully updated.' }
        format.json { render :show, status: :ok, location: @in_ur_ap }
      else
  ...
  # DELETE /in_ur_aps/1
  # DELETE /in_ur_aps/1.json
  def destroy
    @in_ur_ap.destroy
    respond_to do |format|
      format.html { redirect_to in_ur_aps_url, notice: 'In ur ap was successfully destroyed.' }
